    ...
    joinRoom = async(roomId, playerId) => {
        ...
        if(room.players.length < 8){
            ...
            if(room.players.length == 8){
                ...
                await utils.createCollection(this.client, threads.villagerThread,"chat");
                await utils.createCollection(this.client, threads.gameStateThread, "playerState");
                const players = []
                const messages = []
                for(let i=0; i<8; i++){
            ...
            return {roomId, villagerThread: threads.villagerThread}
        }
        ...
        const updater = async() => {
            ...
            switch (room.phase) {
                case "WAITING":
                    room.phase = "NIGHT"
                    room.currentDay = 1
                    await utils.createCollection(this.client, threads.gameStateThread, `NIGHT-${room.currentDay}`)
                    break
                case "NIGHT":
                    room.phase = "DISCUSSION"
                    const nightResult = await game.nightResult(
                        this.client,
                        threads.gameStateThread,
                        `NIGHT-${room.currentDay}`
                    )
                    await playerState.markPlayerDead(this.client, this.threadId, nightResult.deadPlayer)
                    message = {
                        subject: "DEAD_PLAYER",
                        message: nightResult,
                        to: "ALL",
                ...
                case "DISCUSSION":
                    room.phase = "VOTING"
                    await utils.createCollection(this.client, threads.gameStateThread, `DAY-${room.currentDay}`)
                    break
                case "VOTING":
                    room.phase = "NIGHT"
                    room.currentDay += 1
                    await utils.createCollection(this.client, threads.gameStateThread, `NIGHT-${room.currentDay}`)
                    const dayResult = await game.nightResult(
                        this.client,
                        threads.gameStateThread,
                        `DAY-${room.currentDay}`
                    )
                    await playerState.markPlayerDead(this.client, this.threadId, dayResult.deadPlayer)
                    message = {
            ...
            await this.client.save(this.threadId, "rooms", [room])
            winner = await game.checkWinningCondition(this.client, this.threadId)
            if(winner){
                message = {
                    subject: "WINNER",
                    message: winner,
